# evaluation.py (renamed from evaluation_clean.py)
import numpy as np
import glob
import os
import importlib
...
def forward_to_indices(model, batch, z_indices, mask, sampling_params):
    ...
    # This `batch` already contains `text_prompt` if it's set in the main function.
    # The `model.get_xc` in your modified Transformer will extract it.
    xc_output = model.get_xc(batch)
    
    # Ensure text_prompts is extracted correctly from xc_output, which is now a tuple of 3 elements.
    if len(xc_output) == 3:
        x, c, text_prompts_for_sampling = xc_output # Use the specific name expected by model.sample
    else: # Fallback for models not expecting text_prompts
        x, c = xc_output
        text_prompts_for_sampling = None
    ...
    index_sample, probs, candidates = model.sample(
        ...
        text_prompts_for_sampling=text_prompts_for_sampling, # Pass the extracted text prompt
...
def main(args):
    ...
    # Model Checkpoint Paths (defined here for clarity, not directly used for loading if InpaintingMaster handles it)
    ckpt_paths = {
        "transformer": "ckpts/transformer_text.ckpt", # Update to new checkpoint for text transformer
    ...
    if not os.path.exists(args.input_image_path):
        print(f"Error: Input image not found at '{args.input_image_path}'. Please check the path.")
        exit()
    
    img_path = args.input_image_path
    print(f'Processing single image: {img_path} with prompt: "{args.text_prompt}"')
    ...
    mask = box_mask(x_down.shape, _GLOBAL_DEVICE, 0.6, det=True).float()
    ...
    with torch.no_grad():
        ...
        # Create batch with text prompt
        # The text_prompt needs to be a list for the batch dimension
        new_batch = {'image': (x_down * mask).permute(0, 2, 3, 1),
                     'text_prompt': [args.text_prompt]} # <--- Inject the single text prompt here
        ...
        dec, _, mout, f0, f1 = model.current_model(
            new_batch, # <--- Pass new_batch (which contains text_prompt) to current_model (Decoder)
    ...
    output_prefix = os.path.splitext(os.path.basename(img_path))[0] + f'_{args.text_prompt.replace(" ", "_")[:20]}' # Add prompt to filename
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run image inpainting evaluation with text conditioning.")

    # --- Model and System Parameters ---
    parser.add_argument("--config_path", type=str, default="configs/places_inpainting_text.yaml", # <-- 更改預設值
    ...
    # --- Input/Output Paths ---
    parser.add_argument("--input_image_path", type=str, required=True, # <-- 更改為單張圖片路徑，設為必填
                        help="Path to the single input image (e.g., 'dataset/val/some_image.jpg').")
    parser.add_argument("--text_prompt", type=str, required=True, # <-- 新增文本提示參數，設為必填
                        help="Text prompt to condition the inpainting (e.g., 'a cat sitting on a chair').")
    parser.add_argument("--output_folder", type=str, default="my_inpainting_results/semantic/", # <-- 更改預設輸出資料夾
